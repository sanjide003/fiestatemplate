...
exports.studentRegistrationPortal = onCall(async request => {
  ...
  if (action === 'list') {
    ...
    registrationsSnap.docs.forEach(doc => { const item = doc.data(); selectedCounts[item.eventId] = (selectedCounts[item.eventId] || 0) + (item.studentIds || []).length; });const teamApps=await db.collection('registrationApplications').where('team','==',student.team).get(),teamEventCounts={};teamApps.docs.forEach(doc=>{const item=doc.data(),row=teamEventCounts[item.eventId]||={applications:0,selected:0,finalizedParticipants:selectedCounts[item.eventId]||0};row.applications++;if(['selected','finalized'].includes(item.status))row.selected++});const eventRows=eventsSnap.docs.map(doc=>({id:doc.id,...doc.data()})),eventMap=Object.fromEntries(eventRows.map(event=>[event.id,event])),directRegistrations=registrationsSnap.docs.filter(doc=>(doc.data().studentIds||[]).includes(student.id)).map(doc=>{const item=doc.data(),event=eventMap[item.eventId]||{};return{id:`registration_${doc.id}`,eventId:item.eventId,eventName:event.name||item.eventName||'Event',eventSnapshot:event,status:'approved',registrationSource:item.registrationSource||'team_leader',studentId:student.id,team:item.team,createdAt:item.createdAt||item.lastUpdatedAt}});
