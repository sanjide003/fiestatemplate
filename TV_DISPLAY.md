...
Each queued result runs through a full-screen scene sequence: a six-second programme-name introduction, reverse-order position reveals, winner/team names and awarded points. After every result in the batch has been shown, the previous team totals animate to the new totals and the ranked leaderboard remains on screen for the default readable hold period. The display then returns to its normal dashboard. A small badge reports the current result number and batch size.
