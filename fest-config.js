...
const LANDING_ORDER_DEFAULTS={sections:['hero','about','leaders','schedule','gallery'],menu:['results','about','leaders','gallery','schedule','registration','login','teamStandings','talent'],quickButtons:['results','about','leaders','gallery','schedule','registration','login','teamStandings','talent']};
export const DEFAULT_PUBLIC_VIEW = Object.freeze({landing:{sections:{hero:true,about:true,leaders:true,gallery:true,schedule:true,teamStandings:true,social:true,footer:true},orders:LANDING_ORDER_DEFAULTS,menu:{results:true,about:true,leaders:true,gallery:true,schedule:true,registration:true,login:true,teamStandings:false,talent:false},quickButtons:{results:false,about:true,leaders:true,gallery:true,schedule:true,registration:true,login:true,teamStandings:false,talent:false},customLinks:[]},resultsPage:{defaultTab:'result',tabs:{result:true,team:true,talent:true},audiences:{enabled:['student','parent','staff','alumni','public'],default:'student',showAll:true},resultsTab:{showCategories:true,includeGeneral:true,hideEmpty:false,scoreDisplay:'total',cardFields:{category:true,gender:false,team:true,chestNo:true,position:true,grade:true,positionPoints:false,gradePoints:false,totalPoints:true},downloads:{enabled:true,resultCard:true,certificate:true,poster:true}},teamsTab:{cardFields:{rank:true,total:true,medals:true,progress:true},breakdowns:{overall:true,category:true,gender:true,awards:true},showTimeline:true},talentTab:{rankingMode:'category',topCount:3,showViewAll:true,showGenderFilter:true,showParticipantSelector:true,defaultParticipantType:'student',audiences:{student:{enabled:true,groupByCategory:true,groupByGender:true},parent:{enabled:true,groupByCategory:false,groupByGender:true},staff:{enabled:true,groupByCategory:false,groupByGender:true},alumni:{enabled:true,groupByCategory:false,groupByGender:false},public:{enabled:true,groupByCategory:true,groupByGender:false}},cardFields:{team:true,gender:true,category:true,chestNo:true,totalPoints:true,positionPoints:false,gradePoints:false}}}});
const mergePublicView=raw=>{const v=raw||{},l=v.landing||{},r=v.resultsPage||{},rt=r.resultsTab||{},tt=r.teamsTab||{},tal=r.talentTab||{},talentAudiences=Object.fromEntries([...new Set([...Object.keys(DEFAULT_PUBLIC_VIEW.resultsPage.talentTab.audiences),...Object.keys(tal.audiences||{})])].map(key=>[key,{enabled:true,groupByCategory:true,groupByGender:false,...(DEFAULT_PUBLIC_VIEW.resultsPage.talentTab.audiences[key]||{}),...(tal.audiences?.[key]||{})}]));const ordered=(name)=>{const valid=LANDING_ORDER_DEFAULTS[name],saved=Array.isArray(l.orders?.[name])?l.orders[name]:[];return[...new Set([...saved.filter(key=>valid.includes(key)),...valid])];};return{landing:{sections:{...DEFAULT_PUBLIC_VIEW.landing.sections,...(l.sections||{})},orders:{sections:ordered('sections'),menu:ordered('menu'),quickButtons:ordered('quickButtons')},menu:{...DEFAULT_PUBLIC_VIEW.landing.menu,...(l.menu||{})},quickButtons:{...DEFAULT_PUBLIC_VIEW.landing.quickButtons,...(l.quickButtons||{})},customLinks:Array.isArray(l.customLinks)?l.customLinks.filter(item=>String(item?.label||'').trim()&&String(item?.url||'').trim()).map(item=>({label:String(item.label||'').trim(),url:String(item.url||'').trim(),icon:String(item.icon||'')})):[]},resultsPage:{...DEFAULT_PUBLIC_VIEW.resultsPage,...r,tabs:{...DEFAULT_PUBLIC_VIEW.resultsPage.tabs,...(r.tabs||{})},audiences:{...DEFAULT_PUBLIC_VIEW.resultsPage.audiences,...(r.audiences||{})},resultsTab:{...DEFAULT_PUBLIC_VIEW.resultsPage.resultsTab,...rt,cardFields:{...DEFAULT_PUBLIC_VIEW.resultsPage.resultsTab.cardFields,...(rt.cardFields||{})},downloads:{...DEFAULT_PUBLIC_VIEW.resultsPage.resultsTab.downloads,...(rt.downloads||{})}},teamsTab:{...DEFAULT_PUBLIC_VIEW.resultsPage.teamsTab,...tt,cardFields:{...DEFAULT_PUBLIC_VIEW.resultsPage.teamsTab.cardFields,...(tt.cardFields||{})},breakdowns:{...DEFAULT_PUBLIC_VIEW.resultsPage.teamsTab.breakdowns,...(tt.breakdowns||{})}},talentTab:{...DEFAULT_PUBLIC_VIEW.resultsPage.talentTab,...tal,audiences:talentAudiences,cardFields:{...DEFAULT_PUBLIC_VIEW.resultsPage.talentTab.cardFields,...(tal.cardFields||{})}}}};};
...
export const DEFAULT_FEST_SETUP = Object.freeze({
  ...
  publicModules: { home: true, results: true, about: true, leaders: true, gallery: true, schedule: true, registration: false, login: true, teamStandings: true, talentRankings: true, tv: true },
