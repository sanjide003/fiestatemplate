...
window.autoFillSchedule=async()=>{const keys=selectedScheduleRows.size?[...selectedScheduleRows].filter(x=>x.startsWith('event:')):visibleScheduleRowIds.filter(x=>x.startsWith('event:'));if(!keys.length)return window.showToast('Show or select programs first','error');const rows=candidateRows(),days=setup().days,stages=setup().stages,durations=setup().durations;if(!days.length||!stages.length)return window.showToast('Add at least one day and one stage first','error');const targets=keys.map(k=>rows.find(x=>!x.isBreak&&x.id===k.slice(6))).filter(Boolean);const missing={day:0,stage:0,duration:0,start:0};targets.forEach(row=>{if(!row.dayId)missing.day++;if(!row.stageId)missing.stage++;if(!row.duration)missing.duration++;if(!row.time)missing.start++;});const issueLines=Object.entries(missing).filter(([,count])=>count).map(([key,count])=>`${count} program(s) missing ${key}`);if(issueLines.length){const proceed=await window.confirmAction(`Auto Fill will fix these schedule gaps:

${issueLines.join('\n')}

Default choices: first day, first stage, first duration preset, continuous start times. Continue?`);if(!proceed)return;}targets.forEach(row=>{const draft={...(scheduleDrafts.get(row.id)||{})};if(!row.dayId){const day=days[0];draft.dayId=day.id;draft.date=day.date;}if(!row.stageId){const stage=stages[0];draft.stageId=stage.id;draft.stage=stage.name;}if(!row.duration)draft.duration=Number(durations[0]||row.duration||10);scheduleDrafts.set(row.id,draft);});let refreshed=candidateRows();const grouped=new Map();targets.forEach(original=>{const row=refreshed.find(x=>!x.isBreak&&x.id===original.id);if(!row)return;const key=`${row.dayId}|${row.stageId}`;(grouped.get(key)||grouped.set(key,[]).get(key)).push(row);});grouped.forEach(group=>{group.sort((a,b)=>a.baseOrder-b.baseOrder);const day=dayFor(group[0].dayId);let cursor=timeToMinutes(group.find(x=>x.time&&x.manualStart)?.time||day?.startTime||'9:00 AM');group.forEach(row=>{const draft={...(scheduleDrafts.get(row.id)||{})};if(row.time&&row.manualStart)cursor=timeToMinutes(row.time);draft.time=minutesToTime(cursor);draft.manualStart=row.manualStart;draft.scheduleAutoFilled=true;scheduleDrafts.set(row.id,draft);cursor+=Number(draft.duration||row.duration||10);refreshed=candidateRows();refreshed.filter(x=>x.isBreak&&x.afterEventId===row.id&&x.dayId===(draft.dayId||row.dayId)&&((x.stageIds||[]).includes('__all')||(x.stageIds||[]).includes(draft.stageId||row.stageId))).forEach(br=>{const patch={...(scheduleBreakDrafts.get(br.id)||{}),time:minutesToTime(cursor),endTime:minutesToTime(cursor+Number(br.duration||10))};scheduleBreakDrafts.set(br.id,patch);cursor+=Number(br.duration||10);});});});renderScheduleBuilder();window.showToast('Auto Fill completed as private draft. Click Save Now when ready.');};
...
const finalEntryExportEvents=()=>finalEntryFilteredEvents({cat:finalEntryDownloadValue('final-entry-download-category'),stage:finalEntryDownloadValue('final-entry-download-stage'),gender:finalEntryDownloadValue('final-entry-download-gender'),participation:'all'}).filter(event=>registrations.some(reg=>reg.eventId===event.id));
...
const finalEntryRowsForEvent=event=>[...teams].sort().map(team=>{const rows=registrations.filter(reg=>reg.eventId===event.id&&reg.team===team).sort((a,b)=>Number(a.slotIndex||0)-Number(b.slotIndex||0)).flatMap(reg=>[...(reg.studentIds||[]),...(reg.participantIds||[])].map(id=>finalEntryParticipantInfo(reg,id))).filter(row=>row.name);return{team,rows};}).filter(group=>group.rows.length);
const finalEntrySafe=value=>escapeHtml(String(value??''));
const finalEntryPrintHtml=()=>{const category=finalEntryDownloadValue('final-entry-download-category')||'All Categories',stage=finalEntryDownloadValue('final-entry-download-stage'),gender=finalEntryDownloadValue('final-entry-download-gender'),eventsList=finalEntryExportEvents(),logo=homeConfig.logoUrl?`<img class="final-entry-logo" src="${finalEntrySafe(homeConfig.logoUrl)}" alt="Fest logo">`:'';return `<style>.final-entry-sheet{width:794px;min-height:1123px;margin:0 auto;background:#fff;color:#000;padding:20px;font-family:Arial,'Noto Sans Malayalam','Noto Sans',sans-serif}.final-entry-head{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:6px}.final-entry-title{font-size:20px;font-weight:900;line-height:1.1}.final-entry-sub{font-size:11px;font-weight:900;text-transform:uppercase}.final-entry-logo{max-width:112px;max-height:58px;object-fit:contain}.final-entry-grid{display:grid;grid-template-columns:1fr 1fr;gap:4px 12px}.final-entry-event{break-inside:avoid;page-break-inside:avoid}.final-entry-event-title{font-size:12px;font-weight:900;line-height:1.15;margin:0 0 1px}.final-entry-table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9px;line-height:1.08}.final-entry-table th,.final-entry-table td{border:2px solid #111;padding:2px 3px;color:#000;vertical-align:middle}.final-entry-table th{font-size:8px;font-weight:900;text-align:center}.final-entry-team{width:16px;text-align:center;font-weight:900;writing-mode:vertical-rl;transform:rotate(180deg);font-size:8px;letter-spacing:.03em}.final-entry-no{width:22px;text-align:center;font-weight:900}.final-entry-chest{width:40px;text-align:center;font-weight:900}.final-entry-name{font-weight:900;text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.final-entry-remark{width:45px}.final-entry-empty{padding:24px;text-align:center;font-weight:900}</style><div class="final-entry-sheet"><div class="final-entry-head"><div><div class="final-entry-title">${finalEntrySafe(category)}${gender?` (${finalEntrySafe(gender)})`:''}</div><div class="final-entry-sub">${finalEntrySafe([stage,'Final Entries'].filter(Boolean).join(' • '))}</div></div>${logo}</div><div class="final-entry-grid">${eventsList.map(event=>{const groups=finalEntryRowsForEvent(event);return `<section class="final-entry-event"><h4 class="final-entry-event-title">ITEM : ${finalEntrySafe(event.name||'Program')}</h4><table class="final-entry-table"><thead><tr><th></th><th>NO</th><th>CH.NO</th><th>NAME OF PARTICIPANTS</th><th></th></tr></thead><tbody>${groups.map(group=>group.rows.map((row,index)=>`<tr>${index===0?`<td class="final-entry-team" rowspan="${group.rows.length}">${finalEntrySafe(teamLabel(group.team))}</td>`:''}<td class="final-entry-no">${index+1}</td><td class="final-entry-chest">${finalEntrySafe(row.chestNo)}</td><td class="final-entry-name">${finalEntrySafe(row.name)}</td><td class="final-entry-remark"></td></tr>`).join('')).join('')}</tbody></table></section>`}).join('')||'<div class="final-entry-empty">No final entries for selected filters.</div>'}</div></div>`;};
...
window.renderFinalEntriesDownloadPreview=()=>{const host=document.getElementById('final-entries-download-preview'),summary=document.getElementById('final-entries-download-summary');if(!host)return;host.innerHTML=finalEntryPrintHtml();const count=finalEntryExportEvents().length;if(summary)summary.textContent=`${count} programme table(s) match the selected filters.`;};
