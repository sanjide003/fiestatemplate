...
document.addEventListener('click', (event) => {
    ...
    const actions = {
        ...
        'reset-student-participation-report': () => window.resetStudentParticipationReport(),
...
window.renderStudentParticipationReport=()=>{const setOptions=(id,values,label)=>{const el=document.getElementById(id);if(!el)return;const old=el.value;el.innerHTML=`<option value="">All ${label}</option>`+values.map(v=>`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join('');if(values.includes(old))el.value=old};setOptions('student-report-category',[...new Set(students.map(s=>s.category||s.details?.category).filter(Boolean))].sort(),'Categories');setOptions('student-report-class',[...new Set(students.map(studentReportClassName).filter(Boolean))].sort((a,b)=>classSortRank(a)-classSortRank(b)||String(a).localeCompare(String(b))),'Classes');setOptions('student-report-team',[...teams].sort(),'Teams');const rows=studentParticipationReportRows(),groups=groupStudentReportRows(rows);const summary=document.getElementById('student-report-summary');if(summary)summary.innerHTML=[['Students',rows.length],['Events',rows.reduce((s,r)=>s+r.totalEvents,0)],['Points',rows.reduce((s,r)=>s+r.totalPoints,0)],['Warnings',`${rows.warnings?.resultOnly||0} result-only / ${rows.warnings?.missing||0} missing ids`]].map(([l,v])=>`<article class="rounded-2xl border bg-slate-50 p-4"><b class="block text-2xl font-black">${v}</b><span class="text-[10px] font-black uppercase text-slate-500">${l}</span></article>`).join('');const host=document.getElementById('student-report-preview');if(host)host.innerHTML=`<table class="min-w-[1100px] w-full text-left text-xs"><thead class="bg-slate-50 text-[10px] font-black uppercase text-slate-500"><tr>${['Sl No','Chest No','Student Name','Class','Category','Gender','Team','Total Participated Events','Published Result Events','Total Points','First','Second','Third'].map(h=>`<th class="p-3">${h}</th>`).join('')}</tr></thead><tbody class="divide-y">${groups.map(g=>`<tr class="bg-indigo-50"><td colspan="13" class="p-3 font-black text-indigo-800">${escapeHtml(g.label)}</td></tr>${g.rows.map((r,i)=>`<tr><td class="p-3">${i+1}</td><td class="p-3 font-mono">${escapeHtml(r.chestNo||'—')}</td><td class="p-3 font-black">${escapeHtml(r.name)}</td><td class="p-3">${escapeHtml(r.className||'—')}</td><td class="p-3">${escapeHtml(r.category||'—')}</td><td class="p-3">${escapeHtml(r.gender||'—')}</td><td class="p-3">${escapeHtml(teamLabel(r.team)||r.team||'—')}</td><td class="p-3">${r.totalEvents}</td><td class="p-3">${r.publishedResultEvents}</td><td class="p-3 font-black">${r.totalPoints}</td><td class="p-3">${r.first}</td><td class="p-3">${r.second}</td><td class="p-3">${r.third}</td></tr>`).join('')}`).join('')}</tbody></table>`;window.lucide?.createIcons?.();};
const studentReportPdfHtml=(rows)=>`${registrationPdfStyles()}${registrationPdfHeaderHtml('STUDENT PARTICIPATION SUMMARY')}<div class="registration-export-meta">${studentReportFilterSummary().map(([k,v])=>`${registrationEscape(k)}: ${registrationEscape(v)}`).join(' • ')}</div>${groupStudentReportRows(rows).map((g,i)=>`<section class="registration-student-section ${i&&studentReportChecked('student-report-page-break')?'registration-student-page-break':''}"><div class="registration-student-head">${registrationEscape(g.label)}</div><table class="registration-pdf-table"><thead><tr>${['Sl','Chest','Name','Class','Gender','Team','Events','Result Events','Points','1st','2nd','3rd'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${g.rows.map((r,i)=>`<tr><td>${i+1}</td><td>${registrationEscape(r.chestNo)}</td><td>${registrationEscape(r.name)}</td><td>${registrationEscape(r.className)}</td><td>${registrationEscape(r.gender)}</td><td>${registrationEscape(teamLabel(r.team)||r.team)}</td><td>${r.totalEvents}</td><td>${r.publishedResultEvents}</td><td>${r.totalPoints}</td><td>${r.first}</td><td>${r.second}</td><td>${r.third}</td></tr>`).join('')}</tbody></table></section>`).join('')}</div>`;
function studentReportFilterSummary(){return [['Category',studentReportValue('student-report-category')||'All'],['Class',studentReportValue('student-report-class')||'All'],['Gender',studentReportValue('student-report-gender')||'All'],['Team',studentReportValue('student-report-team')||'All'],['Result mode',studentReportValue('student-report-result-status')||'published'],['Group points',studentReportChecked('student-report-group-full-points')?'Full to each member':'Team only'],['Generated',new Date().toLocaleString()]]}
let studentReportPdfBlob=null,studentReportPdfUrl='';
const createStudentParticipationReportPdf=rows=>createRegistrationPdfFromHtml(studentReportPdfHtml(rows));window.createStudentParticipationReportPdf=createStudentParticipationReportPdf;
...
window.exportStudentParticipationReportExcel=()=>{if(!window.XLSX)return window.showToast('SheetJS 0.18.5 is not loaded','error');const rows=studentParticipationReportRows();if(!rows.length)return window.showToast('No student participation rows match the filters','error');const summary=rows.map((r,i)=>({'Sl No':i+1,'Chest No':r.chestNo,Name:r.name,Class:r.className,Category:r.category,Gender:r.gender,Team:teamLabel(r.team)||r.team,'Total Events':r.totalEvents,'Published Result Events':r.publishedResultEvents,'Total Points':r.totalPoints,First:r.first,Second:r.second,Third:r.third}));const grouped=[];groupStudentReportRows(rows).forEach(g=>{grouped.push({Group:g.label});g.rows.forEach((r,i)=>grouped.push({'Sl No':i+1,'Chest No':r.chestNo,Name:r.name,Class:r.className,Category:r.category,Gender:r.gender,Team:r.team,'Total Events':r.totalEvents,'Total Points':r.totalPoints,First:r.first,Second:r.second,Third:r.third}));grouped.push({});});const details=studentParticipationEventDetails({resultStatus:studentReportValue('student-report-result-status'),fullGroupPoints:studentReportChecked('student-report-group-full-points')}).map(d=>{const s=students.find(x=>x.id===d.studentId)||{};return{Chest:s.chestNo||'',Name:s.name||'',Event:d.eventName,Category:d.eventCategory,Stage:d.stage,Position:d.position,Points:d.points,Source:d.source}});const meta=studentReportFilterSummary().map(([Field,Value])=>({Field,Value}));meta.push({Field:'Total students',Value:rows.length},{Field:'Total registrations',Value:registrations.length},{Field:'Total result rows',Value:results.length},{Field:'Result-only students found',Value:rows.warnings?.resultOnly||0},{Field:'Missing result winner ids',Value:rows.warnings?.missing||0});const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(summary),'Summary');XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(grouped),'Grouped Summary');XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(details),'Event Details');XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(meta),'Metadata');XLSX.writeFile(book,'Student_Participation_Summary.xlsx')};
window.resetStudentParticipationReport=()=>['student-report-category','student-report-class','student-report-gender','student-report-team'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';})||window.renderStudentParticipationReport();
