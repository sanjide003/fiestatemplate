...
function candidateRows(){
  const programs=events.filter(event=>event.scheduleRequirement!=='disabled').map((event,index)=>({id:event.id,name:event.name||'',category:event.category||'General',gender:genderLabel(event),stageType:event.stage||'Off-Stage',cancelled:event.cancelled===true,cancelReason:event.cancelReason||'',judgeId:judgeAssignments.find(a=>a.active!==false&&a.eventId===event.id)?.judgeId||'',participantIds:registrations.filter(r=>r.eventId===event.id).flatMap(r=>[...(r.studentIds||[]),...(r.participantIds||[])]),baseOrder:index,...eventDraft(event)}));
...
const finalEntryEventTeams = eventId => [...new Set(registrations.filter(r => r.eventId === eventId && r.team).map(r => r.team))];
...
window.renderRegList = (force = false) => {
    ...
    if(!container) return;
    const currentHash = generateHash(registrations) + generateHash(events) + evId + cat + stage + gender + participation + expandedRegistrationEventId + generateHash(students);
