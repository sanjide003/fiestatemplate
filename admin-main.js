...
async function startAdminSession() {
    ...
    window.adminUnsubscribers.push(onSnapshot(collection(db, "students"), (snap) => { students = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.adminSharedState.update({ students }); window.renderStudentTable(); window.renderAdminEntryManager?.(); window.updateStats(); }));
    ...
    window.adminUnsubscribers.push(onSnapshot(collection(db, "events"), (snap) => { events = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.adminSharedState.update({ events }); window.renderEventTable(); window.renderRegistrationRequests?.(); window.renderAdminEntryManager?.(); const eventFilter = document.getElementById('reg-filter-event'); if(eventFilter?.options.length <= 1) window.populateRegEvents(); else window.renderRegList(true); window.updateStats(); }));
    window.adminUnsubscribers.push(onSnapshot(collection(db, "registrations"), (snap) => { registrations = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.updateStats(); window.renderRegList(); window.renderAdminEntryManager?.(); }));
...
document.addEventListener('click', (event) => {
    ...
    const actions = {
        ...
        'render-admin-entry-manager': () => window.renderAdminEntryManager(),
        'open-admin-entry-event': () => window.openAdminEntryEvent(d.id),
        'edit-admin-entry-slot': () => window.editAdminEntrySlot(d.id),
        'add-admin-entry-slot': () => window.addAdminEntrySlot(),
        'save-admin-entry-slot': () => window.saveAdminEntrySlot(),
        'delete-admin-entry-slot': () => window.deleteAdminEntrySlot(d.id),
        'close-admin-entry-modal': () => window.closeAdminEntryModal(),
...
const adminEntryMemberLimits=event=>{const group=event?.type==='Group',fallback=group?Math.max(1,Number(event?.groupSize||1)):1,min=group?Math.max(1,Number(event?.minimumMembers||fallback)):1,max=group?Math.max(min,Number(event?.maximumMembers||fallback)):1;return{min,max,label:min===max?`${max} member${max===1?'':'s'}`:`${min}-${max} members`};};
const adminEntryPersonIds=registration=>[...(registration?.studentIds||[]),...(registration?.participantIds||[])].filter(Boolean).map(String);
const adminEntryStudent=()=>students.find(item=>item.id===document.getElementById('admin-entry-student')?.value);
const adminEntryTeam=()=>adminEntryStudent()?.team||'';
const adminEntryEventRows=(eventId,team=adminEntryTeam())=>uniqueRegistrationRows().filter(reg=>reg.eventId===eventId&&reg.team===team).sort((a,b)=>Number(a.slotIndex||0)-Number(b.slotIndex||0));
const adminEntryNextSlot=(event,team=adminEntryTeam())=>{const used=new Set(adminEntryEventRows(event.id,team).map(reg=>Number(reg.slotIndex||0)));let slot=1;while(used.has(slot))slot++;return slot;};
const adminEntryEligibleForEvent=(student,event)=>{if(!student?.id||!event?.id||event.status==='draft'||event.cancelled===true)return false;if(!student.team)return false;const types=event.allowedParticipantTypes||festSetup.participantTypes||['student'];if(types.length&&!types.includes('student'))return false;const gender=document.getElementById('admin-entry-gender')?.value||studentGender(student);const genders=[event.gender,event.genderScope].filter(Boolean);if(genders.length&&!genders.includes('Both')&&!genders.includes('Mixed')&&!genders.includes(gender))return false;const category=document.getElementById('admin-entry-category')?.value||student.category||'';if(!eventCategoryEligible({...student,category},event,registrationConfig))return false;if(Array.isArray(event.eligibleClasses)&&event.eligibleClasses.length&&!event.eligibleClasses.map(String).includes(String(student.details?.class||student.class||'')))return false;const stage=document.getElementById('admin-entry-stage')?.value||'';if(stage&&(event.stage||'Off-Stage')!==stage)return false;return true;};
const ensureAdminEntryModal=()=>{let modal=document.getElementById('admin-entry-modal');if(modal)return modal;modal=document.createElement('div');modal.id='admin-entry-modal';modal.className='fixed inset-0 z-[430] hidden items-center justify-center bg-slate-950/70 p-3 backdrop-blur';modal.innerHTML=`<article class="flex max-h-[94dvh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl"><header class="flex items-start justify-between gap-3 border-b bg-emerald-50 p-4"><div><h2 id="admin-entry-modal-title" class="text-lg font-black text-emerald-950">Manage Final Entry</h2><p id="admin-entry-modal-subtitle" class="text-xs font-bold text-emerald-700"></p></div><button data-admin-action="close-admin-entry-modal" class="rounded-xl bg-white px-3 py-2 text-xs font-black text-slate-600">Close</button></header><div id="admin-entry-modal-body" class="min-h-0 flex-1 overflow-y-auto p-4"></div><footer class="flex justify-end gap-2 border-t p-3"><button data-admin-action="close-admin-entry-modal" class="rounded-xl bg-slate-100 px-5 py-3 text-xs font-black text-slate-600">Cancel</button><button data-admin-action="save-admin-entry-slot" class="rounded-xl bg-emerald-600 px-5 py-3 text-xs font-black text-white">Save Entry</button></footer></article>`;document.body.append(modal);return modal;};
let adminEntryModalState={eventId:'',regId:'',slotIndex:0,team:''};
window.renderAdminEntryManager=()=>{const studentSelect=document.getElementById('admin-entry-student'),list=document.getElementById('admin-entry-event-list');if(!studentSelect||!list)return;const selected=studentSelect.value;studentSelect.innerHTML='<option value="">Choose student</option>'+students.slice().sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))).map(student=>`<option value="${escapeHtml(student.id)}" ${student.id===selected?'selected':''}>${escapeHtml(student.name||'Student')} • ${escapeHtml(teamLabel(student.team)||student.team||'No team')} • ${escapeHtml(student.chestNo||'-')}</option>`).join('');const student=adminEntryStudent();const gender=document.getElementById('admin-entry-gender'),category=document.getElementById('admin-entry-category');if(student){if(gender&&!gender.value)gender.value=studentGender(student);if(category&&!category.value)category.value=student.category||'';}if(!student){list.innerHTML='<p class="col-span-full rounded-2xl border border-dashed bg-white p-6 text-center text-xs font-bold text-slate-400">Choose a student to list eligible events.</p>';return;}if(!student.team){list.innerHTML='<p class="col-span-full rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-xs font-black text-red-700">Selected student has no team. Add a team before final entry.</p>';return;}const eligible=events.filter(event=>adminEntryEligibleForEvent(student,event)).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));list.innerHTML=eligible.length?eligible.map(event=>{const rows=adminEntryEventRows(event.id,student.team),limits=adminEntryMemberLimits(event),entryLimit=Math.max(1,Number(event.limit||1)),full=rows.length>=entryLimit;return `<article class="rounded-2xl border bg-white p-4 shadow-sm"><div class="flex items-start justify-between gap-2"><div><h5 class="font-black text-slate-900">${escapeHtml(event.name||'Event')}</h5><p class="mt-1 text-[10px] font-bold uppercase text-slate-400">${escapeHtml(categoryLabel(event.category||'General'))} • ${escapeHtml(event.stage||'Off-Stage')} • ${escapeHtml(genderLabel(event))} • ${escapeHtml(event.type||'Single')}</p></div><span class="rounded-full ${full?'bg-amber-100 text-amber-700':'bg-emerald-100 text-emerald-700'} px-2 py-1 text-[10px] font-black">${rows.length}/${entryLimit} entries</span></div><p class="mt-3 text-xs font-bold text-slate-600">${event.type==='Group'?`Group entry: ${limits.label}`:'Single entry: 1 member'}</p><button data-admin-action="open-admin-entry-event" data-id="${escapeHtml(event.id)}" class="mt-4 w-full rounded-xl bg-emerald-600 px-4 py-3 text-xs font-black text-white">Manage Entry</button></article>`}).join(''):'<p class="col-span-full rounded-2xl border border-dashed bg-white p-6 text-center text-xs font-bold text-slate-400">No eligible events match this student and filters.</p>';window.lucide?.createIcons?.();};
window.openAdminEntryEvent=eventId=>{const student=adminEntryStudent(),event=events.find(item=>item.id===eventId);if(!student||!event)return window.showToast('Choose a student and event first','error');adminEntryModalState={eventId,regId:'',slotIndex:adminEntryNextSlot(event,student.team),team:student.team};ensureAdminEntryModal().classList.remove('hidden');ensureAdminEntryModal().classList.add('flex');window.renderAdminEntryModal();};
window.closeAdminEntryModal=()=>{const modal=ensureAdminEntryModal();modal.classList.add('hidden');modal.classList.remove('flex');};
window.addAdminEntrySlot=()=>{const event=events.find(item=>item.id===adminEntryModalState.eventId);adminEntryModalState={...adminEntryModalState,regId:'',slotIndex:adminEntryNextSlot(event,adminEntryModalState.team)};window.renderAdminEntryModal();};
window.editAdminEntrySlot=regId=>{const reg=registrations.find(item=>item.id===regId);if(!reg)return;adminEntryModalState={eventId:reg.eventId,regId:reg.id,slotIndex:Number(reg.slotIndex||1),team:reg.team};window.renderAdminEntryModal();};
window.deleteAdminEntrySlot=async regId=>{const reg=registrations.find(item=>item.id===regId),event=events.find(item=>item.id===reg?.eventId);if(!reg)return;if(!await window.confirmAction(`Delete Entry ${reg.slotIndex||''} from ${event?.name||'this event'}?`))return;await deleteDoc(doc(db,'registrations',reg.id));window.showToast('Final entry deleted');window.renderAdminEntryManager();window.renderAdminEntryModal();};
window.renderAdminEntryModal=()=>{const modal=ensureAdminEntryModal(),body=document.getElementById('admin-entry-modal-body'),event=events.find(item=>item.id===adminEntryModalState.eventId),student=adminEntryStudent(),team=adminEntryModalState.team||student?.team||'',rows=event?adminEntryEventRows(event.id,team):[],editing=registrations.find(item=>item.id===adminEntryModalState.regId),selected=new Set(adminEntryPersonIds(editing));if(!event||!student){body.innerHTML='<p class="p-6 text-center text-sm font-bold text-red-600">Missing event or student.</p>';return;}const limits=adminEntryMemberLimits(event),entryLimit=Math.max(1,Number(event.limit||1)),slot=adminEntryModalState.slotIndex||adminEntryNextSlot(event,team),eligible=students.filter(item=>item.team===team&&adminEntryEligibleForEvent(item,event)).sort((a,b)=>String(a.chestNo||'').localeCompare(String(b.chestNo||''),undefined,{numeric:true})||String(a.name||'').localeCompare(String(b.name||'')));document.getElementById('admin-entry-modal-title').textContent=`${event.name||'Event'} — ${teamLabel(team)||team}`;document.getElementById('admin-entry-modal-subtitle').textContent=`${event.type||'Single'} • ${limits.label} per entry • ${rows.length}/${entryLimit} entries used`;body.innerHTML=`<div class="grid gap-4 lg:grid-cols-[1fr_1.2fr]"><section class="rounded-2xl border bg-slate-50 p-4"><div class="mb-3 flex items-center justify-between"><h3 class="font-black text-slate-800">Existing entries</h3><button data-admin-action="add-admin-entry-slot" class="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">+ New Entry</button></div>${rows.length?rows.map(row=>{const names=adminEntryPersonIds(row).map(id=>students.find(s=>s.id===id)?.name||participants.find(p=>p.id===id)?.name||row.participantMeta?.[id]?.name||id);return `<article class="mb-2 rounded-xl border bg-white p-3 ${row.id===adminEntryModalState.regId?'ring-2 ring-emerald-400':''}"><div class="flex items-center justify-between gap-2"><b class="text-sm">Entry ${escapeHtml(row.slotIndex||'')}</b><span class="text-[10px] font-black text-slate-400">${names.length}/${limits.max}</span></div><p class="mt-2 text-xs font-bold text-slate-600">${names.map(escapeHtml).join(' • ')||'No members'}</p><div class="mt-3 flex gap-2"><button data-admin-action="edit-admin-entry-slot" data-id="${escapeHtml(row.id)}" class="rounded-lg bg-indigo-50 px-3 py-2 text-[10px] font-black text-indigo-700">Edit</button><button data-admin-action="delete-admin-entry-slot" data-id="${escapeHtml(row.id)}" class="rounded-lg bg-red-50 px-3 py-2 text-[10px] font-black text-red-600">Delete</button></div></article>`}).join(''):'<p class="rounded-xl border border-dashed bg-white p-4 text-xs font-bold text-slate-400">No entries yet.</p>'}</section><section class="rounded-2xl border bg-white p-4"><input id="admin-entry-edit-reg-id" type="hidden" value="${escapeHtml(adminEntryModalState.regId)}"><label class="input-group"><span class="input-label">Entry / Slot number</span><input id="admin-entry-slot-index" type="number" min="1" value="${slot}" class="text-input"></label><p class="my-3 text-xs font-bold text-slate-500">Select ${event.type==='Group'?limits.label:'1 member'} for this entry. Admin override works even if public/team leader registration is locked.</p><div class="max-h-[48vh] space-y-2 overflow-y-auto pr-1">${eligible.map(person=>{const already=registrations.some(reg=>reg.eventId===event.id&&reg.id!==editing?.id&&adminEntryPersonIds(reg).includes(person.id));return `<label class="flex items-center gap-3 rounded-xl border p-3 ${already?'bg-red-50 opacity-60':'hover:bg-emerald-50'}"><input type="checkbox" class="admin-entry-member h-4 w-4" value="${escapeHtml(person.id)}" ${selected.has(person.id)||(!editing&&person.id===student.id)?'checked':''} ${already?'disabled':''}><span class="min-w-0 flex-1"><b class="block truncate text-sm">${escapeHtml(person.name||'Student')}</b><small class="font-bold text-slate-400">${escapeHtml(person.chestNo||'-')} • ${escapeHtml(studentGender(person))} • ${escapeHtml(person.category||'')}</small></span>${already?'<span class="text-[10px] font-black text-red-600">Already in event</span>':''}</label>`}).join('')||'<p class="rounded-xl border border-dashed p-4 text-center text-xs font-bold text-red-500">No eligible team members.</p>'}</div></section></div>`;};
window.saveAdminEntrySlot=async()=>{const event=events.find(item=>item.id===adminEntryModalState.eventId),student=adminEntryStudent();if(!event||!student)return window.showToast('Missing event or student','error');const regId=document.getElementById('admin-entry-edit-reg-id')?.value||'',existing=regId?registrations.find(item=>item.id===regId):null,slotIndex=Math.max(1,Number(document.getElementById('admin-entry-slot-index')?.value||adminEntryNextSlot(event,student.team))),ids=[...document.querySelectorAll('.admin-entry-member:checked')].map(input=>input.value),limits=adminEntryMemberLimits(event);if(ids.length<limits.min||ids.length>limits.max)return window.showToast(`Select ${limits.label} for this entry`,'error');const conflict=registrations.find(reg=>reg.eventId===event.id&&reg.id!==regId&&adminEntryPersonIds(reg).some(id=>ids.includes(id)));if(conflict)return window.showToast('One selected member is already in another entry for this event','error');const entryLimit=Math.max(1,Number(event.limit||1)),rows=adminEntryEventRows(event.id,student.team),slotExists=registrations.find(reg=>reg.eventId===event.id&&reg.team===student.team&&Number(reg.slotIndex||0)===slotIndex&&reg.id!==regId);if(slotExists)return window.showToast(`Entry ${slotIndex} already exists for this team`,'error');if(!existing&&rows.length>=entryLimit&&!await window.confirmAction(`This exceeds the ${entryLimit} entr${entryLimit===1?'y':'ies'} allowed for this event. Add anyway as Admin override?`))return;const now=Date.now(),actor={uid:auth.currentUser?.uid||'',name:'Fest Committee',role:'fest_committee'},previous=existing?.participantMeta||{},participantMeta=Object.fromEntries(ids.map(id=>{const person=students.find(item=>item.id===id)||{};return[id,previous[id]||{name:person.name||'',chestNo:person.chestNo||'',team:person.team||student.team,category:person.category||'',gender:studentGender(person),addedByUid:actor.uid,addedByName:actor.name,addedByRole:actor.role,addedAt:now}]}));const payload={eventId:event.id,eventName:event.name||'',team:student.team,slotIndex,studentIds:ids,participantIds:[],participantMeta,registrationSource:'admin-final-entry-manager',adminOverride:true,managedGender:document.getElementById('admin-entry-gender')?.value||studentGender(student),lastUpdatedByUid:actor.uid,lastUpdatedByName:actor.name,lastUpdatedByRole:actor.role,lastUpdatedAt:now,createdByUid:existing?.createdByUid||actor.uid,createdByName:existing?.createdByName||actor.name,createdByRole:existing?.createdByRole||actor.role,createdAt:existing?.createdAt||now};if(existing)await updateDoc(doc(db,'registrations',existing.id),payload);else await addDoc(collection(db,'registrations'),payload);window.showToast('Final entry saved by Fest Committee');adminEntryModalState={eventId:event.id,regId:'',slotIndex:adminEntryNextSlot(event,student.team),team:student.team};window.renderRegList(true);window.renderAdminEntryManager();window.renderAdminEntryModal();};
