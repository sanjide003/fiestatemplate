...
import { normalizeRegistrationConfig, capacityState, participationUsage, participationLimitDecision } from './registration-utils.js';
...
function candidateRows(){
  const programs=events.filter(event=>event.scheduleRequirement!=='disabled').map((event,index)=>({id:event.id,name:event.name||'',category:event.category||'General',gender:genderLabel(event),stageType:event.stage||'Off-Stage',judgeId:judgeAssignments.find(a=>a.active!==false&&a.eventId===event.id)?.judgeId||'',participantIds:registrations.filter(r=>r.eventId===event.id).flatMap(r=>[...(r.studentIds||[]),...(r.participantIds||[])]),baseOrder:index,...eventDraft(event)}));
...
function renderScheduleBuilder(){syncFilterState();renderMasterSetup();const rows=candidateRows(),validation=scheduleValidation(rows),visible=rows.filter(row=>matchesFilter(row,validation));visibleScheduleRowIds=visible.map(row=>`${row.isBreak?'break:':'event:'}${row.id}`);const body=document.getElementById('schedule-builder-body');if(body)body.innerHTML=visible.length?visible.map(row=>builderRow(row,validation)).join(''):'<tr><td colspan="10" class="p-12 text-center font-bold text-slate-400">No rows match all selected filters.</td></tr>';const scheduled=events.filter(e=>{const row=rows.find(x=>!x.isBreak&&x.id===e.id);return row?.dayId&&row?.stageId&&row?.duration&&row?.time;}).length,set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val;};set('schedule-stat-total',events.length);set('schedule-stat-scheduled',scheduled);set('schedule-stat-unscheduled',events.length-scheduled);set('schedule-stat-breaks',rows.filter(x=>x.isBreak).length);set('schedule-stat-conflicts',[...validation.values()].filter(x=>x.some(y=>y.includes('overlap'))).length);set('schedule-dirty-count',`${dirtyCount()} changes`);updateSelectionUi();syncScheduleStageGroups();window.lucide?.createIcons?.();}
...
window.addScheduleDay=()=>{const label=document.getElementById('setup-day-label')?.value.trim(),date=document.getElementById('setup-day-date')?.value,startTime=document.getElementById('setup-day-start')?.value,endTime=document.getElementById('setup-day-end')?.value;if(!label||!date||!startTime||!endTime||timeToMinutes(endTime)<=timeToMinutes(startTime))return window.showToast('Enter valid day label, date, start and end time','error');scheduleConfig.scheduleDays=[...setup().days,{id:uid('day'),label,date,startTime:toAmPm(startTime),endTime:toAmPm(endTime)}];renderScheduleBuilder();};
window.addScheduleStage=()=>{const name=document.getElementById('setup-stage-name')?.value.trim();if(!name)return window.showToast('Enter stage name','error');if(setup().stages.some(stage=>stage.name.toLowerCase()===name.toLowerCase()))return window.showToast('This stage already exists','error');scheduleConfig.scheduleStages=[...setup().stages,{id:uid('stage'),name}];renderScheduleBuilder();};
window.addScheduleDuration=()=>{const value=Number(document.getElementById('setup-duration-value')?.value||0);if(value<=0)return window.showToast('Enter a valid duration','error');scheduleConfig.durationPresets=[...new Set([...setup().durations,value])].sort((a,b)=>a-b);renderScheduleBuilder();};
...
window.saveScheduleMaster=async()=>{
  ...
  try{
    ...
    window.showToast('Schedule master setup saved to database');
...
window.autoFillSchedule=()=>{const keys=selectedScheduleRows.size?[...selectedScheduleRows].filter(x=>x.startsWith('event:')):visibleScheduleRowIds.filter(x=>x.startsWith('event:'));if(!keys.length)return window.showToast('Select programs or show programs with filters first','error');const rows=candidateRows(),targets=keys.map(k=>rows.find(x=>!x.isBreak&&x.id===k.slice(6))).filter(Boolean);if(targets.some(x=>!x.dayId||!x.stageId||!x.duration))return window.showToast('Select Day, Stage and Duration for every target program','error');const groups=new Map();targets.forEach(row=>{const key=`${row.dayId}|${row.stageId}`;(groups.get(key)||groups.set(key,[]).get(key)).push(row);});groups.forEach(group=>{group.sort((a,b)=>a.baseOrder-b.baseOrder);const day=dayFor(group[0].dayId),stage=stageFor(group[0].stageId);let cursor=timeToMinutes(group.find(x=>x.time)?.time||day?.startTime||'9:00 AM');group.forEach(row=>{const draft={...(scheduleDrafts.get(row.id)||{})};if(row.time&&row.manualStart)cursor=timeToMinutes(row.time);draft.time=minutesToTime(cursor);draft.manualStart=row.manualStart;draft.scheduleAutoFilled=true;scheduleDrafts.set(row.id,draft);cursor+=Number(row.duration||10);const breaks=candidateRows().filter(x=>x.isBreak&&x.afterEventId===row.id&&x.dayId===row.dayId&&((x.stageIds||[]).includes('__all')||(x.stageIds||[]).includes(row.stageId)));breaks.forEach(br=>{const patch={...(scheduleBreakDrafts.get(br.id)||{}),time:minutesToTime(cursor),endTime:minutesToTime(cursor+Number(br.duration||10))};scheduleBreakDrafts.set(br.id,patch);cursor+=Number(br.duration||10);});});});renderScheduleBuilder();};
...
const liveSavedRows=()=>candidateRows().filter(row=>row.isBreak?scheduleBreaks.some(b=>b.id===row.id&&!deletedScheduleBreakIds.has(row.id)):isScheduledEvent(events.find(e=>e.id===row.id)||{})).map(row=>({type:row.isBreak?'break':'program',id:row.id,name:row.name||row.title,category:row.category||'General',gender:row.gender||'—',dayId:row.dayId,date:row.date,stageId:row.stageId,stageIds:row.stageIds||[],stages:row.stages||[],stage:row.stage,duration:Number(row.duration||0),time:row.time,endTime:rowEnd(row),afterEventId:row.afterEventId||''}));
...
const visibleColumns=mode=>{const {order,hidden}=columnSettings(),grouped=groupedColumn(mode),columns=order.filter(column=>!hidden.has(column)&&column!==grouped);return columns.length?columns:['program'];};
...
function reviewRows(){const mode=document.getElementById('schedule-review-mode')?.value||'date',filter=document.getElementById('schedule-review-filter')?.value||'';return versionRows().filter(row=>!filter||String(mode==='date'?row.date:mode==='stage'?row.stage:mode==='category'?row.category:row.gender)===filter);}
...
async function createSchedulePdf(){
  ...
  if(hasMalayalam&&!customFont){
    ...
    if(noteElement){
      ...
      noteCanvas=await window.html2canvas(clone,{scale:2,backgroundColor:'#fff'});
      clone.remove();
      noteElement.style.display='none';
    }
    const paperRect=paper.getBoundingClientRect();
    const canvas=await window.html2canvas(paper,{scale:2,backgroundColor:'#fff'});
    ...
    while(sourceY<canvas.height){
      ...
      docPdf.addImage(slice.toDataURL('image/jpeg',0.96),'JPEG',margin,margin,contentWidth,sliceHeight*scale);
      sourceY=sourceEnd;
    }
    if(noteCanvas&&note.text){
      const noteImage=noteCanvas.toDataURL('image/png');
      notePageNumbers(docPdf.getNumberOfPages(),note.pages).forEach(page=>{docPdf.setPage(page);docPdf.addImage(noteImage,'PNG',margin,pageHeight-margin-noteHeight,contentWidth,noteHeight);});
    }
    renderSchedulePaper();
    return docPdf;
  }
...
window.previewSchedulePdf=async()=>{const modal=document.getElementById('schedule-pdf-modal'),loading=document.getElementById('schedule-pdf-loading'),errorBox=document.getElementById('schedule-pdf-error'),frame=document.getElementById('schedule-pdf-frame');modal.classList.remove('hidden');modal.classList.add('flex');loading.classList.remove('hidden');errorBox.classList.add('hidden');frame.removeAttribute('src');try{revokeSchedulePdf();const pdf=await createSchedulePdf();schedulePdfBlob=pdf.output('blob');schedulePdfUrl=URL.createObjectURL(schedulePdfBlob);frame.src=schedulePdfUrl;document.getElementById('schedule-pdf-title').textContent='Schedule PDF';document.getElementById('schedule-pdf-meta').textContent='Filtered schedule • jsPDF 2.5.1 preview';document.getElementById('schedule-preview-status').textContent='Filtered schedule PDF preview generated';}catch(error){console.error(error);errorBox.textContent=error.message||'Unable to generate PDF';errorBox.classList.remove('hidden');}finally{loading.classList.add('hidden');}};
window.closeSchedulePdf=()=>{document.getElementById('schedule-pdf-modal')?.classList.add('hidden');document.getElementById('schedule-pdf-modal')?.classList.remove('flex');document.getElementById('schedule-pdf-frame')?.removeAttribute('src');revokeSchedulePdf();};
window.downloadSchedulePdf=()=>{if(!schedulePdfBlob)return window.showToast('Generate a PDF preview first','error');const a=document.createElement('a');a.href=schedulePdfUrl;a.download='Fest_Time_Schedule.pdf';a.click();};
window.printSchedulePreview=()=>document.getElementById('schedule-pdf-frame')?.contentWindow?.print();
window.exportScheduleExcel=()=>{if(!window.XLSX)return;const rows=candidateRows().filter(x=>!x.isBreak).map(r=>({Program:r.name,Day:dayFor(r.dayId)?.label||'',Date:r.date,Stage:r.stage,Duration:r.duration,'Start Time':r.time,'End Time':rowEnd(r)})),book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(rows),'Schedule');XLSX.writeFile(book,'Fest_Time_Schedule.xlsx');};
window.exportScheduleReviewExcel=()=>{if(!window.XLSX)return;const mode=document.getElementById('schedule-review-mode')?.value||'date',columns=visibleColumns(mode),labels={category:'Category',program:'Program',gender:'Gender',date:'Date',time:'Start Time',end:'End Time',duration:'Duration',stage:'Stage'},rows=reviewRows().map(row=>Object.fromEntries(columns.map(column=>[labels[column],column==='program'?row.name:column==='duration'?Number(row.duration||0):row[column]||'']))),book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(rows),'Filtered Schedule');XLSX.writeFile(book,'Fest_Filtered_Schedule.xlsx');};
...
window.populateRegEvents = () => {
    ...
    evSelect.innerHTML = '<option value="all">All Registered Events</option>' + filteredEvents.map(e => `<option value="${escapeHtml(e.id)}">${escapeHtml(e.name)}</option>`).join('');
    evSelect.value = 'all';
...
window.renderRegList = (force = false) => {
    const evId = document.getElementById('reg-filter-event').value;
    const cat = document.getElementById('reg-filter-cat').value;
    const stage = document.getElementById('reg-filter-stage').value;
    const container = document.getElementById('registrations-container');
    const currentHash = generateHash(registrations) + generateHash(events) + evId + cat + stage + generateHash(students);
    ...
    const renderTeamCard = (teamName, eventInfo, showEmpty = true) => {
        const teamRegs = registrations.filter(r => r.eventId === eventInfo.id && r.team === teamName).sort((a,b)=>a.slotIndex-b.slotIndex);
        const isRegistered = teamRegs.length > 0;
        if(!isRegistered && !showEmpty) return '';
        let contentHtml = '';
        if (isRegistered) {
            contentHtml = teamRegs.map(r => {
                const studentList = (r.studentIds||[]).map(sid => {
                    const application=registrationApplications.find(item=>item.studentId===sid&&item.eventId===r.eventId),meta=r.participantMeta?.[sid]||{},s=students.find(std=>std.id===sid)||{id:sid,name:meta.name||application?.studentName||'Student',chestNo:meta.chestNo||application?.chestNo||'',team:r.team,category:application?.category||''};
        ...
        return `<div class="rounded-xl border ${cardBorder} ${cardBg} shadow-sm overflow-hidden break-inside-avoid transition hover:shadow-md animate-enter"><div class="px-4 py-3 bg-slate-100/50 border-b border-slate-100 flex justify-between items-center"><h4 class="font-bold ${headerColor} truncate max-w-[70%]">${escapeHtml(teamName)}</h4>${isRegistered ? `<span class="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-0.5 rounded-full">Registered</span>` : ''}</div><div class="p-3">${contentHtml}</div></div>`;
    };

    if(evId && evId !== 'all') {
        const eventInfo = events.find(e => e.id === evId);
        if(!eventInfo) { container.innerHTML = '<p class="py-10 text-center text-sm font-bold text-slate-400">Event not found.</p>'; return; }
        const teamsHtml = sortedTeams.map(teamName => renderTeamCard(teamName, eventInfo)).join('');
        container.innerHTML = `<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">${teamsHtml}</div>`;
    } else {
        let filteredEvents = cat ? events.filter(e => e.category === cat || (cat !== 'General' && e.category === 'General')) : [...events];
        if(stage) filteredEvents = filteredEvents.filter(e => e.stage === stage);
        const registeredEventIds = new Set(registrations.map(r => r.eventId));
        filteredEvents = filteredEvents.filter(e => registeredEventIds.has(e.id)).sort((a,b) => String(a.name || '').localeCompare(String(b.name || '')));
        container.innerHTML = filteredEvents.length ? filteredEvents.map(eventInfo => {
            const registeredTeams = sortedTeams.filter(teamName => registrations.some(r => r.eventId === eventInfo.id && r.team === teamName));
            return `<section class="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div class="mb-4 flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3"><div><h3 class="font-black text-slate-800">${escapeHtml(eventInfo.name)}</h3><p class="text-[11px] font-bold uppercase text-slate-400">${escapeHtml(eventInfo.category || 'General')} • ${escapeHtml(eventInfo.stage || 'Off-Stage')}</p></div><span class="rounded-full bg-indigo-50 px-3 py-1 text-[11px] font-black text-indigo-700">${registeredTeams.length} team${registeredTeams.length === 1 ? '' : 's'}</span></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">${registeredTeams.map(teamName => renderTeamCard(teamName, eventInfo, false)).join('')}</div></section>`;
        }).join('') : '<div class="py-12 text-center text-sm font-bold text-slate-400">No registered events found for the selected filters.</div>';
    }
    window.lucide?.createIcons?.();
};
...
window.approveRegistrationRequest = async id => {
    ...
    (request.eventIds||[]).forEach((eventId,index)=>{const event=events.find(item=>item.id===eventId),ids=event?.type==='Group'?[participantRef.id,...memberRefs.map(ref=>ref.id)]:[participantRef.id];batch.set(doc(collection(db,'registrations')),{eventId,team:resolvedTeam,slotIndex:index+1,participantIds:isStudent?[]:ids,studentIds:isStudent?[related.id]:[],registrationSource:'self',createdByUid:'public-request',createdAt:now,lastUpdatedAt:now});});
...
const renderParticipantEligibleEvents=()=>{const box=document.getElementById('participant-eligible-events');if(!box)return;const type=masterValue('participant-type-admin'),gender=masterValue('participant-gender-admin'),category=masterValue('participant-category-admin'),studentId=document.getElementById('participant-student-id')?.value,participantId=document.getElementById('participant-edit-id')?.value,eligible=events.filter(event=>event.type!=='Group'&&event.status!=='draft'&&(event.allowedParticipantTypes||festSetup.participantTypes).includes(type)&&(!gender||event.gender==='Both'||event.gender===gender)&&(!category||event.category==='General'||event.category===category));box.innerHTML=eligible.map(event=>{const registered=registrations.some(reg=>reg.eventId===event.id&&(studentId?(reg.studentIds||[]).includes(studentId):participantId?(reg.participantIds||[]).includes(participantId):false));return`<label class="rounded-xl border bg-white p-3 text-xs font-bold"><input type="checkbox" name="participant-events" value="${event.id}" class="mr-2" ${registered?'checked disabled':''}>${escapeHtml(event.name)}${registered?' • Registered':''}</label>`;}).join('')||'<p class="col-span-full py-3 text-center text-xs font-bold text-slate-400">No eligible individual events.</p>';};
...
window.renderJudgeAssignmentOptions = () => {
    ...
    table.innerHTML = `<table class="w-full min-w-[900px] text-left text-sm"><thead class="bg-slate-50 text-[11px] font-black uppercase tracking-wider text-slate-500"><tr><th class="px-4 py-3">Date & Day</th><th class="px-4 py-3">Time</th><th class="px-4 py-3">Program</th><th class="px-4 py-3">Gender</th><th class="px-4 py-3">Stage</th><th class="px-4 py-3">Assigned Judge</th></tr></thead><tbody class="divide-y divide-slate-100">${list.map(e => { const a = assignmentForEvent(e.id); return `<tr><td class="px-4 py-3 font-bold">${escapeHtml(scheduleDateText(eventScheduleDate(e)))}</td><td class="px-4 py-3 font-black text-indigo-700">${escapeHtml(toAmPm(eventScheduleTime(e) || ''))}</td><td class="px-4 py-3"><div class="font-black">${escapeHtml(e.name || '')}</div><div class="text-xs text-slate-400 font-bold">${escapeHtml(e.category || '')} • ${escapeHtml(eventStageType(e))}</div></td><td class="px-4 py-3 font-bold">${escapeHtml(genderLabel(e))}</td><td class="px-4 py-3 font-bold">${escapeHtml(eventScheduleStage(e) || '-')}</td><td class="px-4 py-3"><select data-assign-event-id="${escapeHtml(e.id)}" class="text-input min-w-[220px]">${judgeOptionsForAssignment(a?.judgeId || '')}</select></td></tr>`; }).join('') || '<tr><td colspan="6" class="px-4 py-10 text-center text-slate-400 font-bold">No scheduled programs.</td></tr>'}</tbody></table>`;
};
...
window.saveJudgeAssignment = async () => {
    ...
    if(issues.length) return window.alert(`Assignment conflicts found:\n\n${issues.map((x,i)=>`${i+1}. ${x}`).join('\n')}`);
...
window.renderJudgeAssignments = () => {
    ...
    list.innerHTML = active.length ? `<h4 class="font-black text-slate-800">Current Assignments</h4>` + active.map(a => { const score = judgeScoreForAssignment(a.id); const status = score?.status || 'pending'; const badge = status === 'submitted' ? 'bg-green-100 text-green-700' : status === 'draft' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'; return `<div class="rounded-2xl border border-slate-200 bg-white p-4 flex flex-col md:flex-row md:items-center justify-between gap-3"><div><h4 class="font-black text-slate-800">${escapeHtml(a.eventName || '')}</h4><p class="text-xs font-bold text-slate-500">${escapeHtml(scheduleDateText(a.date))} • ${escapeHtml(toAmPm(a.time || ''))} • Stage ${escapeHtml(a.stage || '-')} • Judge: ${escapeHtml(a.judgeName || '')}</p></div><div class="flex flex-wrap items-center gap-2"><span class="rounded-full px-3 py-1 text-[10px] font-black uppercase ${badge}">${status}</span><button data-admin-action="edit-judge-assignment" data-id="${escapeHtml(a.id)}" class="rounded-lg bg-indigo-50 px-3 py-2 text-xs font-black text-indigo-700 hover:bg-indigo-100"><i data-lucide="pencil" class="inline-block w-3 h-3 mr-1"></i>Edit</button><button data-admin-action="remove-judge-assignment" data-id="${escapeHtml(a.id)}" class="rounded-lg bg-red-50 px-3 py-2 text-xs font-black text-red-600 hover:bg-red-100"><i data-lucide="trash-2" class="inline-block w-3 h-3 mr-1"></i>Delete</button></div></div>`; }).join('') : '<p class="text-sm text-slate-400 italic">No assignments yet.</p>';
};
